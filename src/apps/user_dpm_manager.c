...
extern void holdDpmAbnormalCheck();
...
static UINT	dpmControlManagerDoneFlag = 0;
...
#if 0	// Will be supported next step
static int externalWakeup = 0;
#endif	// 0
...
#if !defined (__LIGHT_DPM_MANAGER__)
static session_config_t *getSessionInfoPtrByType(UINT session_type);
#endif	//__LIGHT_DPM_MANAGER__
...
static UINT dmSecureRecv(SECURE_INFO_T *config, void *packet_ptr, ULONG wait_option);
...
int dpm_mng_init_rng();
int dpm_mng_deinit_rng();
...
int	getPacketSendingStatus(void	*confPtr, UINT type)
{
#if !defined (__LIGHT_DPM_MANAGER__)
...
int	getPacketReceivingStatus(void *confPtr, UINT type)
{
#if !defined (__LIGHT_DPM_MANAGER__)
	tcpRxSubTaskConf_t	*tcpRxSubTaskConfPtr;
	udpSubSessConf_t	*udpSubSessInfoPtr;
...
dpm_user_config_t *dpm_get_user_config()
{
	return &dpmUserConfigs;
}

dpm_user_config_t *dpm_get_rtm_user_config()
{
...
{
	DM_DBG_INFO(" [%s] call regist function (argc:%d) \n", __func__, argc);

	if (callback == NULL)
	{
		DM_DBG_ERR(" [%s] Not regist callback function (argc:%d) \n", __func__, argc);
		return;
	}
...
static UINT generateClientLocalPort(UINT session_type)
{
	...
	if (session_type != REG_TYPE_TCP_CLIENT
			&& session_type != REG_TYPE_UDP_CLIENT)
	{
	...
	do
	{
		...
		if ((sessConfPtr->sessionType == session_type)
...
void connectCallback(session_config_t *sessConfPtr, UINT status)
{
	...
	if ((sessConfPtr->sessionNo == SESSION1)
			&& (dpmUserConfigs.sessionConfig[0].sessionConnectCallback))
	{
		executeUserCallback(dpmUserConfigs.sessionConfig[0].sessionConnectCallback, 3, paramData, pdTRUE);
	}
	else if ((sessConfPtr->sessionNo == SESSION2)
			 && (dpmUserConfigs.sessionConfig[1].sessionConnectCallback))
	{
		executeUserCallback(dpmUserConfigs.sessionConfig[1].sessionConnectCallback, 3, paramData, pdTRUE);
	}
	else if ((sessConfPtr->sessionNo == SESSION3)
			 && (dpmUserConfigs.sessionConfig[2].sessionConnectCallback))
	{
		executeUserCallback(dpmUserConfigs.sessionConfig[2].sessionConnectCallback, 3, paramData, pdTRUE);
	}
	else if ((sessConfPtr->sessionNo == SESSION4)
			 && (dpmUserConfigs.sessionConfig[3].sessionConnectCallback))
	{
...
int	rtcRegistTcpServerKeepaliveTimer(tcpRxSubTaskConf_t *tcpRxSubTaskConfPtr)
{
	...
	tcpRxSubTaskConfPtr->subSessionKaTid = rtc_register_timer(sessConfPtr->sessionKaInterval,
															tcpRxSubTaskConfPtr->tcpRxSubTaskName,
															tcpRxSubTaskConfPtr->subSessionKaTid,
															0,
															functionPtr);

	DM_DBG_INFO(CYAN " [%s] regist Keep-alive sub_idx:%d name:%s ka_interval:%d ka_tid:%d \n" CLEAR,
															__func__,
...
int	rtcRegistTcpClientKeepaliveTimer(session_config_t *sessConfPtr)
{
	...
	sessConfPtr->sessionKaTid = rtc_register_timer(sessConfPtr->sessionKaInterval,
...
#if 0 // FOR_DEBUGGING
void print_sock_info(int sock_fd)
{
	...
	lwip_getpeername(sock_fd, (struct sockaddr *)&name, &namelen);

	ip = htonl(name.sin_addr.s_addr);
	port = htons(name.sin_port);

	PRINTF(GREEN " [%s] fd:%d ip:0x%x peer_port:%d \n" CLEAR, __func__, sock_fd, ip, port);
}
#endif
...
UINT tls_restore_ssl(char *name, SECURE_INFO_T *config)
{
	...
	//DM_DBG_INFO(" [%s:%d] Restore tls session(%s) \n", __func__, __LINE__, name);
...
static void dm_dtls_timer(TimerHandle_t xTimer)
{
	int ret = 0;
	SECURE_TIMER *timer_ptr;

	timer_ptr = (SECURE_TIMER *)pvTimerGetTimerID(xTimer);
	if (timer_ptr == NULL)
	{
		DM_DBG_ERR(" [%s:%d] Invaild info \n", __func__, __LINE__);
		return;
	}

#ifdef DTLS_TIMER_LOG
	DM_DBG_INFO(" [%s:%d] timer(%s), int_ms(%d), fin_ms(%d)\n",
			__func__, __LINE__, timer_ptr->name, timer_ptr->int_ms, timer_ptr->fin_ms);
#endif

	if (timer_ptr->fin_ms > timer_ptr->int_ms)
	{
		// intermediate expiry
		timer_ptr->fin_ms -= timer_ptr->int_ms;
		timer_ptr->state = INT_EXPIRY;
	}
	else
	{
		timer_ptr->fin_ms = 0;
		...
		if (timer_ptr->timer)
		{
			ret = xTimerDelete(timer_ptr->timer, 0);
			if (ret != pdPASS)
			{
				DM_DBG_ERR(RED " [%s] Failed to delete timer(%d) \n" CLEAR, __func__, ret);
			}
			timer_ptr->timer = NULL;
		}

		timer_ptr->state = FIN_EXPIRY;
	}
...
static void dmDtlsTimerStart(void *ctx, uint32_t int_ms, uint32_t fin_ms)
{
	...
	{
#ifdef DTLS_TIMER_LOG
		DM_DBG_INFO(" [%s:%d] to activate timer(0x%x->%s)\n", __func__, __LINE__, timer_ptr, timer_ptr->name);
#endif

		//delete previous timer
		if (timer_ptr->state == FIN_EXPIRY)
		{
			if (timer_ptr->timer)
				xTimerDelete(timer_ptr->timer, 0);
		}

		timer_ptr->int_ms = int_ms;
		timer_ptr->fin_ms = fin_ms;
		timer_ptr->state = NO_EXPIRY;

		timer_ptr->timer = xTimerCreate(timer_ptr->name,
									(int_ms / 10),
									pdTRUE,
									(void *)timer_ptr,
									(TimerCallbackFunction_t) dm_dtls_timer);
		if (timer_ptr->timer == NULL)
		{
			DM_DBG_ERR(" [%s:%d] Failed to create timer(0x%02x)\n", __func__, __LINE__);
			return;
		}
		else
		{
			xTimerStart(timer_ptr->timer, 0);
		}
	}
	else if (fin_ms == 0)
	{
#ifdef DTLS_TIMER_LOG
		DM_DBG_INFO(" [%s:%d] to deactivate timer(%s)\n", __func__, __LINE__, timer_ptr->name);
#endif

		timer_ptr->state = CANCELLED;
		timer_ptr->int_ms = 0;
		timer_ptr->fin_ms = 0;
		if (timer_ptr->timer)
		{
			status = xTimerDelete(timer_ptr->timer, 0);
			if (status != pdPASS)
			{
				DM_DBG_ERR(RED " [%s] Failed to delete timer(%d) \n" CLEAR, __func__, status);
			}
...
#ifdef DTLS_TIMER_LOG
	DM_DBG_INFO(" [%s:%d] timer(%s), state(%d)\n", __func__, __LINE__, timer_ptr->name, timer_ptr->state);
#endif

	return (int)timer_ptr->state;
}
...
static int dmSecureSendCallback(void *ctx, const unsigned char *buf, size_t len)
{
	...
	//DM_DBG_INFO(" [%s] Start (ptr:0x%x sessConfPtr->socket:%d buf:0x%x len:%d type:%d) \n",
	//		__func__, sessConfPtr, sessConfPtr->socket, buf, len, sessConfPtr->sessionType);
	...
	else
	{
		...
		errorCallback(ERR_ARG, "Fail session type");
		return ERR_ARG;
	}
}
...
static int dmSecureRecvCallback(void *ctx, unsigned char *buf, size_t len)
{
	...
	//DM_DBG_INFO(CYAN " [%s] Called sub_idx:%d len:%d \n" CLEAR, __func__, config->subIndex, len);
	...
	if (sessConfPtr->sessionType == REG_TYPE_TCP_SERVER)
	{
		...
		{
			//DM_DBG_ERR(RED " [%s] ER_NO_PACKET (%d) \n" CLEAR, __func__, ret);
			clrPacketReceivingStatus(tcpRxSubTaskConfPtr, REG_TYPE_TCP_SERVER);
			//xSemaphoreGive(sessConfPtr->socketMutex);
	...
	else if (sessConfPtr->sessionType == REG_TYPE_UDP_SERVER)
	{
		...
		if (ret < 0)
		{	// TIMEOUT: -1
			//DM_DBG_ERR(RED " [%s:%d] recvfrom error status:%d \n" CLEAR, __func__, __LINE__, ret);
...
static UINT dmSecureRecv(SECURE_INFO_T *config, void *packet_ptr, ULONG wait_option)
{
	...
	{
		//DM_DBG_INFO(" [%s:%d] Set read timeout(%ld)\n", __func__, __LINE__, wait_option);
...
static int dmSecureRecvTimeoutCallback(void *ctx, unsigned char *buf, size_t len, uint32_t timeout)
{
	...
	else if (sessConfPtr->sessionType == REG_TYPE_UDP_SERVER)
	{
		...
		mbedtls_net_recv_timeout(config->client_ctx, buf, len, timeout);	//mbedtls_net_bsd_recv_timeout
	}
	...
	else if (sessConfPtr->sessionType == REG_TYPE_UDP_CLIENT)
	{
		...
		mbedtls_net_recv_timeout(config->server_ctx, buf, len, timeout);	// mbedtls_net_bsd_recv_timeout
	}
	else
	{
		DM_DBG_ERR(RED " [%s] Type error (%d)\n" CLEAR, __func__, sessConfPtr->sessionType);
...
void initDoneSubTask()	// TCP Server Only
{
	int	index;
	tcpRxSubTaskConf_t	*tcpRxSubTaskConfPtr;
...
void	clearNotActTcpSubTask()
{
	...
	for(index = 0; index < DM_TCP_SVR_MAX_SESS; index++)
	{
		...
		if (tcpRxSubTaskConfPtr->runStatus == SESSION_STATUS_STOP)
		{
			...
			//DM_DBG_INFO(" [%s] Clear sub task info (index:%d) \n", __func__, index);
		}
	}
}
...
tcp_server_sub_restart:
	...
	if (tcp_rx_pkt == NULL)
	{
		...
		tcp_rx_pkt = tcpRxSubTaskConfPtr->tcpServerSessDataBuffer;
	}
...
#if defined ( SUPPORT_RTM_TCP_KEEPALIVE )
				// Save TCP keep-alive timer id to RTM area
				dpm_tcp_socket_keep_alive_delete(tcpRxSubTaskConfPtr->tcpRxSubTaskName,
												tcpRxSubTaskConfPtr->subSessionKaTid);
				dpm_tcp_socket_keep_alive_set(tcpRxSubTaskConfPtr->tcpRxSubTaskName,
												tcpRxSubTaskConfPtr->subSessionKaTid);
#endif	// SUPPORT_RTM_TCP_KEEPALIVE
...
static ULONG createtcpServerSubTask(session_config_t *sessConfPtr, int sub_idx)
{
	...
	status = xTaskCreate(tcpServerSubTask,
						...
						tskIDLE_PRIORITY + TCP_SERVER_SESSION_TASK_PRIORITY,
...
tcp_server_restart:
	...
	//printSessionTcpSubIndex();		// FOR_DEBUG
...
wait_tcp_accept:
	//DM_DBG_INFO(CYAN " [%s] Wait accept (sock:%d) \n" CLEAR, __func__, sessConfPtr->socket);
	...
	if (client_sock < 0)
	{
		//DM_DBG_INFO(" [%s] accept timeout (createdSubTaskCount:%d acceptedSockCount:%d) \n",
		//								__func__, createdSubTaskCount, acceptedSockCount);
		clearNotActTcpSubTask(sessConfPtr);
...
wait_stop:
	...
	status = createtcpServerSubTask(sessConfPtr, sub_idx);		// tcpServerSubTask
	if (status != ERR_OK)
	{
...
static void dpmTcpServerManager(void *sessionNo)
{
	...
	status = runTcpServer(sessConfPtr);
...
confirm:
	...
	if (sessConfPtr->supportSecure)
	{
		...
		if (config->ssl_ctx->state == MBEDTLS_SSL_HANDSHAKE_OVER)
		{
			...
			status = dmSecureSend(config, data_buf, len_buf, TCP_CLIENT_DEF_WAIT_TIME);
			if (status)
			{
...
static int runTcpClient(session_config_t *sessConfPtr)
{
	...
	UINT		windowSize = TCP_CLIENT_WINDOW_SIZE;
...
tcp_client_restart:
	...
	tcp_server_port = sessConfPtr->sessionServerPort;
	tcp_local_port = sessConfPtr->localPort;
...
retryQuery:
	...
	tcp_server_ip_addr = iptolong(ip_addr_str);
...
tcpc_retry:
	...
	if (sessConfPtr->sessionWindowSize)
	{
		windowSize = sessConfPtr->sessionWindowSize;
	}
	...
	if (sessConfPtr->supportSecure)
	{
		...
		status = dmSecureInit(config);	// TCP CLIENT
		if (status)
		{
			DM_DBG_ERR(" [%s] Failed to init ssl(0x%02x) \n", __func__, status);
		...
		status = tls_restore_ssl(sessConfPtr->sessTaskName, config);
		if (status)
		{
			DM_DBG_ERR(" [%s:%d] Failed to restore ssl(0x%02x)\n", __func__, __LINE__);
...
conn_retry:
	...
	srv_addr.sin_port = htons(tcp_server_port);
	...
	if (sessConfPtr->supportSecure)
	{
		status = dmSecureHandshake(config);	// TCP Client handshake
		if (status)
		{
		...
		status = tls_save_ssl(sessConfPtr->sessTaskName, config);
		if (status)
		{
			DM_DBG_ERR(" [%s:%d] Failed to save tls session(0x%02x)\n", __func__, __LINE__, status);
		}
	...
	while (1)
	{
			...
			(getPacketReceivingStatus(sessConfPtr, REG_TYPE_TCP_CLIENT)) )
		{
			if (delayCountBySendingFlag++ < 100)
			{
				//DM_DBG_INFO(" [%s] send/recv pending (rf:%d sf:%d) \n",
				//	__func__, sessConfPtr->receivingFlag, sessConfPtr->sendingFlag);
				continue;
			}
			else
			{
...
TC_taskEnd:
	...
	{
		// Delete TCP Filter for dpm
		dpm_tcp_port_filter_delete(sessConfPtr->localPort);	// del_dpm_tcp_port_filter
		dpm_app_unregister(sessConfPtr->dpmRegName);
	}
	vTaskDelete(NULL);
}
	...
	if (sessConfPtr->supportSecure)
	{
		...
		if (udpSubSessInfoPtr->runStatus == SESSION_STATUS_STOP)	// not yet received
		{
			...
			if (config->ssl_ctx->state == MBEDTLS_SSL_HANDSHAKE_OVER)
			{
				...
				sprintf((char *)config->timer.name, "%s_%s", UDP_SERVER_DPM_SECURE_NAME, "timer");
...
wait_accept:
	...
	status = setsockopt(config->client_ctx->fd,
						SOL_SOCKET,
						SO_RCVTIMEO,
						(const void *)&tv,
...
#if 0 // FOR_DEBUGGING
	print_sock_info(config->client_ctx->fd);
#endif
...
handshake_over:
	...
	status = setsockopt(udpSubSessInfoPtr->clientSock,
						SOL_SOCKET,
						SO_RCVTIMEO,
						(const void *)&tv,
						sizeof(struct timeval));
...
static int runUdpServer(session_config_t *sessConfPtr)
{
	...
	if (sessConfPtr->supportSecure)		// Secure UDP Server
	{
		...
		//printSessionUdpSubIndex();		// FOR_DEBUG
...
wait_udp_accept:
		...
		//DM_DBG_INFO(" [%s] Wait accept : socket(%d) \n", __func__, listen_socket);
...
udp_server_restart:
	...
	status = setsockopt(sessConfPtr->socket,
						SOL_SOCKET,
						SO_RCVTIMEO,
						(const void *)&tv,
						sizeof(struct timeval));
...
static void dpmUdpServerManager(void *sessionNo)
{
	...
	DM_DBG_INFO(CYAN " [%s] Started dpmUdpServerManager session no:%d \n" CLEAR,
																__func__, (UINT)sessionNo);
	status = runUdpServer(sessConfPtr);
...
static int sendUdpClient(session_config_t *sessConfPtr, char *data_buf, UINT len_buf)
{
	...
	if (sessConfPtr->supportSecure)
	{
		...
		if (config->ssl_ctx->state == MBEDTLS_SSL_HANDSHAKE_OVER)
		{
			status = dmSecureSend(config, data_buf, len_buf, UDP_CLIENT_DEF_WAIT_TIME);
			if (status)
			{
				DM_DBG_ERR(RED " [%s] Failed UDP send packet(buf:0x%x len:%d) \n" CLEAR,
																		__func__, data_buf, len_buf);
...
static void dpm_udp_to_cb(char *timer_name)
{
	...
	dpm_udp_tid = rtc_register_timer(UDP_PERIOD_SEC,
									 sessConfPtr->dpmRegName,
									 dpm_udp_tid,
									 0,
									 dpm_udp_to_cb);
...
UINT dmSecureUdpClientHandshake(SECURE_INFO_T *config)		// UDP CLIENT HANDSHAKE
{
	...
	status = setsockopt(config->server_ctx->fd,
						SOL_SOCKET,
						SO_RCVTIMEO,
						(const void *)&tv,
						sizeof(struct timeval));
...
udp_client_restart:
	...
	ip_addr_str = sessConfPtr->sessionServerIp;
	udp_server_port = sessConfPtr->sessionServerPort;
	udp_local_port = sessConfPtr->localPort;
	...
	if (sessConfPtr->supportSecure)
	{
		...
		status = dmSecureInit(config);	// UDP CLIENT
		if (status)
		{
			DM_DBG_ERR(" [%s] Failed to init ssl(0x%02x) \n", __func__, status);
		...
		local_addr.sin_family = AF_INET;
		local_addr.sin_addr.s_addr = htonl(INADDR_ANY);
		local_addr.sin_port = htons(sessConfPtr->localPort);
		...
		memset(&config->timer, 0x00, sizeof(SECURE_TIMER));
		sprintf((char *)config->timer.name, "%s_%s", UDP_CLIENT_DPM_SECURE_NAME, "timer");
		mbedtls_ssl_set_timer_cb(config->ssl_ctx,
								&config->timer,
								dmDtlsTimerStart,
								dmDtlsTimerGetStatus);
...
UC_taskEnd:
	if (dpm_mode_is_enabled() == DPM_ENABLED)
	{
		dpm_udp_port_filter_delete(sessConfPtr->localPort);		// del_dpm_udp_port_filter
		dpm_app_unregister(sessConfPtr->dpmRegName);
	}
...
#if 0	// Will be supported next step
static void setExternalWakeupEvent(int eventType)
{
	DM_DBG_INFO("\n setExternalWakeupEvent \n\n");
	externalWakeup ++;
}
#endif	// 0
...
static void startSessTask(UINT type, UINT sessionNo)
{
	...
	if (type == REG_TYPE_TCP_CLIENT)
	{
		...
		if (dpm_mode_is_enabled() == DPM_ENABLED)
		{
			...
			//Set TCP Filter for dpm
			dpm_tcp_port_filter_set(sessConfPtr->localPort);	// set_dpm_tcp_port_filter
	...
	else if (type == REG_TYPE_UDP_CLIENT)
	{
		...
		if (dpm_mode_is_enabled() == DPM_ENABLED)
		{
			...
			dpm_udp_port_filter_set(sessConfPtr->localPort);		// set_dpm_udp_port_filter
			DM_DBG_INFO(CYAN " [%s] Set UDP Filter Port %d \n" CLEAR,
					__func__, (unsigned short)sessConfPtr->localPort);
		}
	}
	...
	status = xTaskCreate(functionPtr,
					(CHAR *)sessConfPtr->sessTaskName,
					sessConfPtr->sessionStackSize,
					(void *)sessionNo,
					taskPriority,
					NULL);
}
...
static void registTimer(void)
{
	...
	for (timerNo = 0; timerNo < MAX_TIMER_CNT; timerNo++)
	{
		if (dpmUserConfigs.timerConfig[timerNo].timerType)
		{
			...
			{
				DM_DBG_ERR(RED " [%s] Regist timer: %s,%d \n" CLEAR, __func__,
			...
			status = rtc_register_timer(dpmUserConfigs.timerConfig[timerNo].timerInterval,
										DPM_EVENT_REG_NAME,
										timerBaseID + timerNo,
										period,
										functionPtr);
			...
			{
				DM_DBG_INFO(" [%s] Regist timer - name:%s, id:%d(%d) type:%d, period:%d interval:%d \n",
													__func__,
													DPM_EVENT_REG_NAME,
													timerBaseID + timerNo,
													status,
...
boot:
		...
		if (dpm_mode_is_enabled())
		{
			if (assignDpmRtmArea(DPM_CONFIG_NAME,
								 (void **)&rtmDpmConfigPtr,
								 sizeof(dpm_user_config_t)) == pdTRUE)
			{
	...
	else
	{
		...
		ret = readFromRtm(DPM_CONFIG_NAME,
						(void *)&dpmUserConfigs,
						(void **)&rtmDpmConfigPtr,
						sizeof(dpm_user_config_t));
	...
	if (!dpm_mode_is_wakeup())
	{
		...
		{
			errorCallback(DM_ERROR_NO_BOOT_INIT_CALLBACK, "Not regist bootInitCallback");
		}
	...
	else	// DPM Wake-up
	{
		...
		{
			errorCallback(DM_ERROR_NO_WAKEUP_INIT_CALLBACK, "Not regist wakeupInitCallback");
		}
...
static session_config_t *getSessionInfoPtrByType(UINT session_type)
{
	...
	for (sessionNo = 0; sessionNo < MAX_SESSION_CNT; sessionNo++)
	{

		if (dpmUserConfigs.sessionConfig[sessionNo].sessionType == session_type)
		{
			return (&dpmUserConfigs.sessionConfig[sessionNo]);
		}
	}
...
void dpm_mng_print_session_info(void)
{
	...
	for (int sessionNo = 1; sessionNo <= MAX_SESSION_CNT; sessionNo++)
	{
...
int dpm_mng_stop_session(UINT sessionNo)
{
	...
	DM_DBG_INFO(" [%s] Request to stop the task \"%s\" status:0x%x \n",
						__func__, sessConfPtr->sessTaskName, status);
...
int dpm_mng_set_DPM_timer(UINT id, UINT type, UINT interval, void (*timerCallback)())
{
	...
	{
		DM_DBG_ERR(RED " [%s] Regist timer: %d,%d \n" CLEAR, __func__, id, type);
		return 1;
	}
	...
	rtc_cancel_timer(timerID);

	rtc_register_timer(timerConfPtr->timerInterval,
					DPM_EVENT_REG_NAME,
					timerID,
					period,
					functionPtr);

	DM_DBG_INFO(" [%s] Regist timer: %s,%d \n", __func__, DPM_MANAGER_REG_NAME, timerConfPtr->timerInterval);
...
int dpm_mng_unset_DPM_timer(UINT id)
{
	...
	DM_DBG_INFO(" [%s] Stop regist timer:%s id:%d(%d) \n", __func__, DPM_MANAGER_REG_NAME, id, timerID);
...
#define CHG_PRIO	31
	...
	//dpm_user_timer_list_print();		// FOR_DEBUG
	//prvPrintTimerListsItem();			// FOR_DEBUG
	//printSessionTcpSubIndex();		// FOR_DEBUG
	//printSessionUdpSubIndex();		// FOR_DEBUG
	//vTaskDelay(3);					// FOR_PRINT
