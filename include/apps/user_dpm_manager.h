...
typedef struct secure_timer {
	...
	UINT				fin_ms;
	TimerHandle_t		timer;
	SECRE_TIMER_STATE	state;
	CHAR				name[SECURE_MAX_NAME];
} SECURE_TIMER;
...
#define	DPM_CONTROL_MANAGER_PRI					(23)

#define	DPM_EVENT_NAME							"dpm_event"
#define	DPM_EVENT_MNG_STK						(128*4)
#define	DPM_EVENT_MANAGER_PRI					(12)

#define	TCP_SERVER_SESSION_TASK_PRIORITY		11
#define	UDP_SERVER_SESSION_TASK_PRIORITY		10
#define	TCP_CLIENT_SESSION_TASK_PRIORITY		9
#define	UDP_CLIENT_SESSION_TASK_PRIORITY		8
 ...
 *********************************************************************************
 */
int dpm_mng_send_to_session(UINT sessionNo,
							ULONG ip,
							ULONG port,
							char *buf,
							UINT size);
...
/**
 *********************************************************************************
 * @brief		UDP Holepunch Payload Definition
 * @param[in]	payload:	Address of payload
 * @param[in]	size:		Size of payload
 * @return		0 on success
 *********************************************************************************
 */
int dpm_set_env_udph_user_data_addr(char *payload, int size);
